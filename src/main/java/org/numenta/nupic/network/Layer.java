...
public class Layer<T> implements Persistable {
     ...
     * This layer's thread
     */
    private static transient volatile Thread LAYER_THREAD;
    ...
    private static volatile Queue<Runnable> outStreams = new ConcurrentLinkedQueue<>();
    private static volatile AtomicInteger threadCount = new AtomicInteger();
    private static volatile AtomicInteger layersCount = new AtomicInteger();

    /**
     * Starts this {@code Layer}'s thread
     */
    protected void startLayerThread() {
        HTMSensor<?>.Copy outputStreamCopy = sensor.getOutputStreamCopy();
        outStreams.add(() -> {
            ...
            if (!outputStreamCopy.hasNext()) {
                return;
            }

            int[] intArray = outputStreamCopy.next();
